...
unsafe impl FromBytes for [u16] {
    ...
    const MIN_LAYOUT: Layout = unsafe { Layout::from_size_align_unchecked(0, mem::align_of::<u16>()) };
    unsafe fn ptr_cast(source: *const [u8]) -> *const Self {
...
unsafe impl FromBytes for [u8] {
    // Allow for empty slices but require correct alignment
    const MIN_LAYOUT: Layout = unsafe { Layout::from_size_align_unchecked(0, mem::align_of::<u8>()) };
...
unsafe impl<T> FromBytes for T where T: Pod {
