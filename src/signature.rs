...
impl SignaturePadding {
    fn to_ffi_args<'a>(&self, out: &'a mut WideCString) -> (PaddingInfo<'a>, u32) {
        match self {
            SignaturePadding::Pkcs1(Pkcs1Padding { algorithm }) => {
                *out = WideCString::from(algorithm.as_str());
                (
                    PaddingInfo {
                        pkcs: BCRYPT_PKCS1_PADDING_INFO {
                            pszAlgId: out.as_ptr(),
...
pub fn sign_hash(key: &KeyHandle, padding: Option<SignaturePadding>, input: &[u8]) -> Result<Box<[u8]>> {
    let mut hash_alg_id = WideCString::new();
    let padding = padding.map(|x| x.to_ffi_args(&mut hash_alg_id));
    let padding_info = padding.as_ref().map(|(padding, _)| padding as *const _ as *mut _);
...
mod tests {
    ...
    fn hash_sign_verify() {
        ...
        let signature = key.sign(&*digest, Some(padding)).expect("Signing to succeed");
        key.verify(&digest, &signature, Some(padding)).expect("Signature to be valid");

        key.verify(&[0xDE, 0xAD], &signature, Some(padding)).expect_err("Bad digest");
        key.verify(&digest, &[0xDE, 0xAD], Some(padding)).expect_err("Bad signature");
        let padding_sha1 = SignaturePadding::pkcs1(Sha1);
        let padding_pss = SignaturePadding::pss(Sha256, 64);
        key.verify(&digest, &signature, Some(padding_sha1)).expect_err("Bad padding");
        key.verify(&digest, &signature, Some(padding_pss)).expect_err("Bad padding");

        let key = AsymmetricKey::builder(Ecdsa(NistP256)).build().unwrap();
        let signature = key.sign(&*digest, None).expect("Signing to succeed");
        key.verify(&digest, &signature, None).expect("Signature to be valid");
        key.verify(&[0xDE, 0xAD], &signature, None).expect_err("Bad digest");
        key.verify(&digest, &[0xDE, 0xAD], None).expect_err("Bad signature");
    }
}
