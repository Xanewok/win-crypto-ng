...
use crate::property::{self, Access, BlockLength, KeyLength, KeyLengths, ObjectLength, MessageBlockLength};
...
impl SymmetricAlgorithmKey {
    ...
    pub fn set_msg_block_len(&mut self, len: usize) -> Result<()> {
        self.handle.set_property::<MessageBlockLength>(&(len as u32))
    }
    ...
    /// ```
    pub fn encrypt(&self, iv: Option<&mut [u8]>, padding: bool, data: &[u8]) -> Result<Buffer> {
        ...
        unsafe {
            Error::check(BCryptEncrypt(
                ...
                if padding { BCRYPT_BLOCK_PADDING } else { 0 },
            ))?;
    ...
    /// ```
    pub fn decrypt(&self, iv: Option<&mut [u8]>, padding: bool, data: &[u8]) -> Result<Buffer> {
        let (iv_ptr, iv_len) = iv
            .map(|iv| (iv.as_mut_ptr(), iv.len() as ULONG))
            .unwrap_or((null_mut(), 0));
...
mod tests {
    ...
    fn check_encryption_decryption(
        ...
        let ciphertext = key.encrypt(iv_cloned().as_deref_mut(), false, data).unwrap();
        let plaintext = key.decrypt(iv_cloned().as_deref_mut(), false, ciphertext.as_slice()).unwrap();
