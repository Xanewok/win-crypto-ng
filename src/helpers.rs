...
pub trait Handle {
    ...
    fn get_property_arr<T>(&self, property: &str) -> Result<TypedBlob<[T]>> {
        ...
        Ok(unsafe { TypedBlob::<[T]>::from_box(result) })
    }
}
...
impl<T> TypedBlob<[T]> {
    ...
    ///
    /// # Panics
    /// This function panicks if the allocation will not hold exactly a certain
    /// count of elements of type `T` or if the backing pointee alignment is not
    /// compatible with that of `&[T]`.
    pub unsafe fn from_box(allocation: Box<[u8]>) -> Self {
        ...
        assert_eq!(allocation.len() % std::mem::size_of::<T>(), 0);
        // Verify that we can produce a valid reference (which are required to
        // always be well-aligned).
        assert_eq!(
...
impl<T> AsRef<[T]> for TypedBlob<[T]> {
    ...
    fn as_ref(&self) -> &[T] {
        ...
        // 1. the caller has to prove that the data is of correct format,
        // 2. we check that the resulting reference will be well-aligned, and
        // 3. the allocation fits exactly a certain count of `T`-sized values.
...
mod tests {
    ...
    fn typed_blob() {
        ...
        let typed = unsafe { TypedBlob::<[u8]>::from_box(bytes.clone()) };
        assert_eq!(typed.as_ref(), bytes.as_ref());
        let typed = unsafe { TypedBlob::<[u16]>::from_box(bytes.clone()) };
