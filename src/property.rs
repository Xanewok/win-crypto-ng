...
fn get_property_size(handle: BCRYPT_HANDLE, prop: LPCWSTR) -> Result<ULONG> {
    ...
    unsafe {
        Error::check(BCryptGetProperty(handle, prop, ptr::null_mut(), 0, &mut size, 0))?;
    }
    Ok(size)
}
...
impl Property for InitializationVector {
    const IDENTIFIER: &'static str = BCRYPT_INITIALIZATION_VECTOR;
    type Value = [u8];
}
...
impl Property for MessageBlockLength {
    const IDENTIFIER: &'static str = BCRYPT_MESSAGE_BLOCK_LENGTH;
    type Value = DWORD;
}
